...
let event_loop rend =
  ...
  | Error (`Msg e) -> Util.log_err " Sdl.set_render_draw_color: %s" e; ()
  | Ok () ->
      match Sdl.render_clear rend with
      | Error (`Msg e) -> Util.log_err " Sdl.render_clear: %s" e; ()
      | Ok () ->
          Sdl.render_present rend;
          let e = Sdl.Event.create () in
          let rec loop () = match Sdl.wait_event (Some e) with
          | Error (`Msg e) -> Util.log_err " Could not wait event: %s" e; ()
          | Ok () ->
              Util.log "%a" Fmts.pp_event e;
...
let main () =
  ...
  | None -> Util.log_err " main: failed to initialize window"
  | Some win ->
